...
-- qual o nome do funcionário que vendeu o produto morango e quantidade da venda e qual é o fornecedor desse produto.
-- pergunta realizada para tabela de vendas 
select usu.nome as 'nome do funcionario',
prod.descricao as 'nome do produto',
vend.quantidade as 'quantidade da venda',
forn.nome as 'nome do fornecedor'
from tbVendas as vend
inner join tbUsuarios as usu on usu.codusu = vend.codusu
inner join tbFuncionarios as func on func.codfunc = usu.codfunc
inner join tbProdutos as prod on prod.codProd = vend.codProd
inner join tbFornecedores as forn on prod.codForn = forn.codForn
where vend.codProd = 2;
